...
MODELS = [
 ...
 m("claude-opus-5-5","anthropic",3.6,18,cr=0.18,cw5=4.5,cw1=7.2,ver="TSF",notes=["不支持 JSON Schema 与 `json_object`","`tool_choice` 用 `\"auto\"`，不支持 `any` / `tool`","始终进行思考，不能关闭"],notes_en=["JSON Schema and `json_object` are not supported","use `tool_choice: \"auto\"`; `any` and `tool` are not supported","thinking is always on and cannot be disabled"]),
 m("claude-opus-5","anthropic",4.50,22.50,cr=0.45,cw5=5.625,cw1=9.00,ver="TSF",limits=("128K","16K"),
   notes=["不支持 JSON Schema 与 `json_object`"],
   notes_en=["JSON Schema and `json_object` are not supported"]),
 m("claude-sonnet-5","anthropic",1.80,9.00,cr=0.18,cw5=2.25,cw1=3.60,ver="TSF",limits=("1M","128K"),
   notes=["不支持 JSON Schema 与 `json_object`"],
   notes_en=["JSON Schema and `json_object` are not supported"]),
