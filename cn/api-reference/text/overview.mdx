...
- `qwen3.8-2.4t-a95b` — 有独立缓存写入价；`tool_choice` 用 `"auto"`
- `qwen3.8-27b` — 有独立缓存写入价；`tool_choice` 用 `"auto"`
- `qwen3.8-flash` — 有独立缓存写入价；`tool_choice` 用 `"auto"`
- `qwen3.7-plus` — 256K 长上下文分档
- `qwen3.7-flash` — 32K 长上下文分档；有独立缓存写入价
