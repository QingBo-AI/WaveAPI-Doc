...
<ParamField body="aspect_ratio" type="string">
  ...
  `wan2.6-i2v` 与 `wan2.6-i2v-flash` 不接受这个参数，画幅跟随输入图。另外几种情况下这个参数也不能传：`wan2.5-preview` 与 `wan2.6` 带 `image_urls` 时、`wan2.7` 带 `first_frame_image` / `image_urls` / `video_urls` 时、`wan2.7-r2v` 带 `first_frame_image` 时——这些场景的画幅跟随输入素材。
</ParamField>
...
| `wan2.5-preview` | 每次任务 | 该模型只生成带声视频，`generate_audio` 不能为 `false` |
| `wan2.5-preview` | 传了 `image_urls` | 图生视频的画幅跟随输入图片，不能指定 `aspect_ratio` |
...
| `wan3.0-video` / `wan3.0-video-prime` | 传了 `video_urls` | 带参考视频时输出最长 15 秒（也可不传 `duration` 或传 `0`，由模型自定时长） |
| `wan3.0-video` / `wan3.0-video-prime` | 传了 `file_url` | `file_url` 与 `link_url` 不能同时提供 |

## 计费

全系按**分辨率档 × 输出秒数**计费，带输入视频时另按**输入秒数**加收一笔。

| 模型 ID | 输出秒数 | 输入视频秒数 | 其他 |
|---|---|---|---|
| `wan2.5-preview` `wan2.6` `wan2.6-i2v` | 请求的 `duration` | — | — |
| `wan2.6-i2v-flash` | 请求的 `duration` | — | 有声输出走带声档 |
| `wan2.7` | 请求的 `duration`；带参考视频时按参考视频档 | 参考视频实际时长 | — |
| `wan2.7-r2v` | 请求的 `duration`；带参考视频时按参考视频档 | 参考视频合计时长，最多按 5 秒计 | — |
| `wan2.7-videoedit` | 请求的 `duration`（`0` 为源视频时长），按参考视频档 | 源视频实际时长 | — |
| `wan3.0-video` / `wan3.0-video-prime` | 请求的 `duration`；`0` 或不传时为实际出片秒数 | 参考视频实际时长 | 不传 `resolution` 按 `480p` 档 |

**带声档**（`wan2.6-i2v-flash`）：开启音频生成（`generate_audio: true`，该模型默认即为 `true`）或传了 `audio_urls` 时按带声档计。

**输入视频秒数**在提交时读取；`wan2.7-videoedit` 的源视频读不到时长时返回 `400`（`input_video_duration_unknown`），不计费。读取规则见[视频生成概述](/cn/api-reference/video/overview#输入视频的时长与计费)。
...
| `wan3.0-video` / `wan3.0-video-prime` | `480p` `720p` `1080p` | `2`–`30`，`0` = 模型自定 | `image_urls` ≤10 · `first_frame_image` / `last_frame_image` · `video_urls` ≤5 · `audio_urls` ≤5 · `file_url` / `link_url` |
