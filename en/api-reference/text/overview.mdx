...
- `gpt-6-sol` — 272K long-context tier; has a separate cache-write rate; max input 922K, max output 128K; function calling requires `reasoning_effort: "none"`; when `reasoning_effort` is not `none`, `temperature`, `top_p` and `logprobs` are not accepted
- `gpt-6-luna` — 272K long-context tier; has a separate cache-write rate; max input 922K, max output 128K; function calling requires `reasoning_effort: "none"`; when `reasoning_effort` is not `none`, `temperature`, `top_p` and `logprobs` are not accepted
