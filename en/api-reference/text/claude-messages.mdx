...
The native Anthropic Messages endpoint, kept for apps already built on the Anthropic SDK. Every text model except the GPT-5.6 / GPT-6 series and the Responses-only models can be called here. It exposes nothing beyond what Chat offers, so new integrations should use the [General Chat API](/en/api-reference/text/general-chat).
