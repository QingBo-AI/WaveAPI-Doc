---
title: "GPT Image 2.5 Official"
description: "Five quality levels for text-to-image, reference editing and masked inpainting, on the Flare and Sunburst lines, billed from token usage"
api: "POST https://api.qingbo.ai/v1/tasks"
---

GPT Image 2.5 is the next generation of the official GPT Image line. It keeps the actions and parameters of GPT Image 2, widens quality from three levels to five, and re-scales the output usage of the levels that share a name. Two models are available, `gpt-image-2.5-flare` (the Flare line) and `gpt-image-2.5-sunburst` (the Sunburst line); their request shape, value ranges and billing are identical, and they differ only in the trade-off between speed and editing precision. Switching between them means changing the `model` field alone.

<Note>
For a fixed price per image, use [GPT Image 2.5 Reverse](/en/api-reference/image/gpt-image/gpt-image-2.5-rev). Both lines use the same task endpoint, but their model IDs, parameters and billing differ, and QWave never switches between them automatically.
</Note>
...
curl -X POST https://api.qingbo.ai/v1/tasks \
  -H "Authorization: Bearer $WAVE_API_KEY" \
  -H "Content-Type: application/json" \
  -H "Idempotency-Key: image25-mask-001" \
  -d '{
    "model": "gpt-image-2.5-sunburst",
    "action": "edit",
    "prompt": "Change only the teacup to a deep green glaze and leave everything else unchanged",
    "image_urls": ["https://cdn.example.com/source.png"],
...
<ParamField body="mask_url" type="string">
  Inpainting mask image URL: a PNG with an alpha channel whose fully transparent pixels mark the area to edit. Its pixel size must match the first reference image, and `image_urls` is required alongside it.
</ParamField>
...
<ParamField body="quality" type="string" default="low">
  Quality level: `low`, `medium`, `high`, `xhigh` or `max`. `xhigh` and `max` exist only on 2.5; sending them to `gpt-image-2` returns `400` and is not downgraded silently. Quality has the largest effect on cost on this line — see Pricing.
</ParamField>
...
| `xhigh` / `max` quality | Accepted only by `gpt-image-2.5-flare` and `gpt-image-2.5-sunburst` |
...
|---|---|---|---|
| `gpt-image-2.5-flare` | Flare line, faster output | Everyday high-quality output, batch generation, quick drafts | From actual token usage |
...
Both models carry the same rates, so the same set of parameters costs the same on either one; only the generated result differs. Every section of this page applies to both.
