...
<ParamField body="duration" type="integer" default="0">
  Omit it or send `0`. The model picks the video length from the content (3–10 seconds); steer pacing with natural language in `prompt`. Any other value returns `400` and is not billed.
</ParamField>
...
`seed` is not supported. Any other parameter returns `400` and is not billed.

## Limits

- One video per task; at most one reference video.
- A reference video and `ref_task_id` are mutually exclusive.
- This model picks the video length itself (3-10 s); duration cannot be set. Omit it or send 0.
...
| Duration | Model-chosen, 3–10 seconds; `duration` omitted or `0` | Model-chosen, 3–10 seconds; `duration` omitted or `0` |
